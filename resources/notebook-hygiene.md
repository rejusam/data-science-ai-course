...
generated by a build script rather than typed by hand. They say so at the top.
The scripts are not in this repository, so an edit you make to one of those
notebooks is overwritten the next time it is rebuilt, without a warning and
without a conflict. Work on your own copy in `my-work/`, and tell the trainer
if something in a session notebook needs changing.
